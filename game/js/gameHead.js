...
let contract_address = "0x504295F3f77e99310B1981D600C85E0d26e141f4".toLowerCase();
...
async function fetchAccountData() {
  ...
  //document.querySelector("#myref").textContent = `https://teng.hu/CRO/index.php?ref=${selectedAccount}`;
	...
	document.querySelector("#total-bet").textContent = (totalBetsHead/1e18).toFixed(2)+' Token';
	...
	if (allowance>0){
		document.querySelector("#random").hidden=false;
...
async function random(){
  ...
  /*var getError = function(e){
      ...
      try {
		 ...
		 let tx = await contract.methods.randomHeadGame(bet,amount)
		 //console.log('venne');
		 ...
		 .once('receipt', function(receipt){
		 		...
		 		let r = receipt['events']['resultHead']['returnValues']['r'];
		 		let user = receipt['events']['resultHead']['returnValues']['user'];
		 		let value = receipt['events']['resultHead']['returnValues']['value'];
		 		let bet = receipt['events']['resultHead']['returnValues']['bet'];
		 		let winAmount = receipt['events']['resultHead']['returnValues']['winAmount']/1e18;
				...
				if (r==1){
		 		...
		 		if (r==value){
					...
					if (r==1){
		 				...
		 				document.querySelector("#result").textContent = "LOST the win: Head";
					}else{

						document.querySelector("#result").textContent = "LOST the win: Tail";
					}
		 		}
		 })
...
async function random2(){
  const web3 = new Web3(provider);
  const contract  = new web3.eth.Contract(contract_abi, contract_address);
  let rand = await contract.methods.randomCubeTest().call();
  document.querySelector("#message").textContent = rand;
  console.log('lefut');
}
...
async function random3(){
  ...
  let rand = await contract.methods.randomCubeGame(amount).call();
  document.querySelector("#message").textContent = rand;
  console.log('lefut2');
}
...
async function calc(){
  ...
  document.querySelector("#amount").innerHTML=`Token: ${amount}`;
...
async function myt(){
  ...
  try {
    const contract  = new web3.eth.Contract(contract_abi, contract_address);
		const result = await contract.getPastEvents('resultDice',
	 {
		 filter: { 0: '0x7e54c562F38D1D904431974755aD965b97817a42' },
		 fromBlock: 18271742,
		 toBlock: 'latest',
	 });
	 console.log(result);
...
function check(){
	if (parseInt(document.querySelector("#amount").value)>=parseInt(minAmount/1e18) && parseInt(document.querySelector("#amount").value)<=parseInt(maxAmount/1e18)){
	...
	document.querySelector("#wamount").textContent = document.querySelector("#amount").value*1.98;
}

function max(){
	document.querySelector("#amount").value = document.querySelector("#max-amount").textContent;
	document.querySelector("#wamount").textContent = document.querySelector("#amount").value*1.98;
}
