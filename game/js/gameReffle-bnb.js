...
let contract_address = "0xa75Cac107989a0d17D798AB1B7C39B3D6ad65D85".toLowerCase();
...
//	document.querySelector("#max-amount").textContent = maxAmount/1e18;
  ...
  //document.querySelector("#tPrize").textContent = getTokenPrize/1e18;

	let getBNBPrize = await contract.methods.getBNBPrize().call();
  document.querySelector("#tPrize").textContent = getBNBPrize/1e18;

	game = await contract.methods.game().call();

	let yourTicket = await contract.methods.playerTickets(selectedAccount,game).call();
	document.querySelector("#your-tickets").textContent = yourTicket;

	let once = await contract.methods.once().call();

	if (once && yourTicket=='1'){
		document.querySelector("#buy").disabled = true;
	}else{
		document.querySelector("#buy").disabled = false;
	}

	let prizeType = await contract.methods.prizeType().call();
	prizeType = (prizeType) ? 'BNB' : 'Token';
	document.querySelector("#prizeType").textContent = prizeType;
...
/*
	let winnerAddress = await contract.methods.winner().call();
	...
	if (now<receivableTime){
		if (winnerAddress===selectedAccount){
			document.querySelector("#tClaim").style.display = "block";
		}else{
			document.querySelector("#tClaim").style.display = "none";
		}
		winnerAddress = (winnerAddress!=0) ? "Winner address: "+winnerAddress : "";
		document.querySelector("#winnerAddress").innerHTML = winnerAddress;
		setInterval(function() {countdown(receivableTime)},1000);
	}else{
		setInterval(function() {countdown(receivableTime)},1000);
	}*/
	...
	setInterval(async function(){
		...
		document.querySelector("#balance").innerHTML = (balance/1e18).toFixed(4);
		...
		if (now<receivableTime){
			...
			document.querySelector("#tClaim").style.display = "none";
			document.querySelector("#winnerAddress").innerHTML = '';
			//setInterval(function() {countdown(receivableTime)},1000);
		}
...
async function buy(){
  ...
  /*var getError = function(e){
     ...
     let tx = await contract.methods.buyTicket()
     //console.log('venne');
...
function leaderboard(){
	let rArray = [];
	$.post( {
			 url: "https://api.bscscan.com/api?module=logs&action=getLogs&fromBlock=19000000&toBlock=99999999&address=0xa75cac107989a0d17d798ab1b7c39b3d6ad65d85&topic0=0xa2e797e41b4733516e86a04377f698b24b02a36e789115662de34478d368db03&apikey=YourApiKeyToken",
			 dataType: "json",
			 data: {},
